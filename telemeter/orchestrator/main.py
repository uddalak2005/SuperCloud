...
# Base directory (stable inside Docker)
...
def handle_backend_response(response: dict):
    ...
    response = response.get("orchestrator_response", response)
    action = response.get("action")

    if action == "alert":
        print("ALERT:", response.get("parameters"))
        return

    if action == "execute_fix":
        print("Fix triggered by orchestrator → fixer service will handle remediation")
        return
